...
async def get_anime(anime_id: int = Path(description="The ID of the anime you want to view.", gt=0)):
    return anime_titles[anime_id]

@app.get("/get-anime-by-name/")
async def get_anime(name: Optional[str] = None):
...
async def create_anime(anime_id: int, anime: Anime):
...
@app.put("/save-anime/{anime_id}")
async def update_anime(anime_id: int, anime: Anime):
    return {"anime_name": anime.name, "anime_id": anime_id}